function [ img ] = xilin_cluster_single(w,ind_img,no_cluster)

% w = distance matrix (sparse)
% ind_img = image where each value is indexed into matrix (image where voxels are numbered in some order and correspond to elements in w)
%    w(4,7) is similarity between voxel #4 and voxel #7
% no_cluster = number of clusters to generate
...
disp(['Creating Label Image ', num2str(len)]);
for i=1:len;
    cur = full(Discrete(i,:));
    aa = find(cur);
    if( length(aa)==1)
