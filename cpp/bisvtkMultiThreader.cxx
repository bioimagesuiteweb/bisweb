                                          ...
                                          vtkThreadFunctionType f, void *data )
{
  ...
  {
    std::cerr << "Can't set method " << index <<
                   " with a thread count of " << this->NumberOfThreads << std::endl;
  }
  else
  {
...
void vtkMultiThreader::SingleMethodExecute()
{
  ...
  {
    std::cerr << "No single method set!" << std::endl;
    return;
  }
  ...
  for ( thread_loop = 1; thread_loop < this->NumberOfThreads; thread_loop++ )
  {
    ...
    if (threadError != 0)
    {
      std::cerr<< "Unable to create a thread.  pthread_create() returned "
                    << threadError<< std::endl;
    }
  }
...
void vtkMultiThreader::MultipleMethodExecute()
{
  ...
  for ( thread_loop = 0; thread_loop < this->NumberOfThreads; thread_loop++ )
  {
    ...
    {
      std::cerr << "No multiple method set for: " << thread_loop << std::endl;
      return;
    }
  }
...
int vtkMultiThreader::SpawnThread( vtkThreadFunctionType f, void *userdata )
{
  ...
  {
    std::cerr << "You have too many active threads!" << std::endl;
    return -1;
  }
...
void vtkMultiThreader::TerminateThread( int threadID )
{
  ...
  {
    std::cerr << "ThreadID is out of range. Must be less that " << VTK_MAX_THREADS << std::endl;
    return;
  }
...
bool vtkMultiThreader::IsThreadActive( int threadID )
{
  // check if the threadID argument is in range
  if ( threadID >= VTK_MAX_THREADS )
  {
...
//----------------------------------------------------------------------------
bool vtkMultiThreader::ThreadsEqual(vtkMultiThreaderIDType t1,
...
void runMultiThreader(vtkThreadFunctionType func, void *ds,std::string msg,int NumberOfThreads,int debug) {
