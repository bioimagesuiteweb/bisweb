...
>>>>>>> b17c00aee3e6e6320e85988f7f6f6db2efe66a27
		...
		//	    height = dim[1] - 150;
		    let svgWidth = width,
			    svgHeight = height,
			    outerRadius = Math.min(width, height) / 4 - 35,
...
<<<<<<< HEAD
	var createChordDialog=function(name,height=-1,width=-1) {
		...
		if (width>0)
			dim[0]=width;
		if (internal.chordDialog) {
			internal.chordDialog.getContainingFrame().remove();
		}
		console.log('Creating chordDialog=',dim);
		internal.chordDialog = bis_webutil.createdialog(name, dim[0], dim[1], 0, 0, 50);
		return dim;
	};
	...
	var drawchords = function() {
		...
		let pos=internal.conndata.createLinePairs(0,internal.laststate.matrixthreshold);
		let mat = internal.conndata.posMatrix;
		let sum_ = mat.reduce(function(a,b) { return a.concat(b) }) // flatten array
			.reduce(function(a,b) { return a + b })/2; 
		console.log('internal.conndata:');	
		var arr= new Array(sum_);
		let c=0;
		//console.log(internal.conndata.posMatrix.length);
		console.log(parseInt(mat[0,0]));
		...
		//pos = internal.conndata.posMatrix;
		console.log(arr);
		//console.log(internal.conndata.posMatrix);
		//internal.laststate.degreethreshold = 0;
...
module.exports= ConnectivityControlElement;
