...
const bisGUIConnectivityControl = function(parent,orthoviewer,layoutmanager) {
    ...
    var corrmap = function() {
	    ...
	    var container = d3.select('.modal-body')
		    ...
		    .attr("float","left")
		    .attr("id","visualcontainer");

	    var legend = d3.select('.modal-body')
		    .append("div")
		    .attr("display","inline-block")
		    .attr("id","visuallegend");


	    var svg = internal.conndata.plotCorrMap(internal.parcellation,
		    pos,
		    internal.laststate.poscolor,
		    internal.context,
		    internal.laststate.length*internal.parcellation.scalefactor,
...
//create modal the size of the doc and size chord drawing to it
		...
		let heatMapDialog = bis_webutil.createdialog('Correlation Map', dim[0], dim[1], 0, 0, 100, () => {
			let frame = heatMapDialog.getContainingFrame();
			frame.remove();
		});

		let width = dim[0] - 50,
			height = dim[1] - 50,
			svgModal = $(heatMapDialog.getContainingFrame().find('.modal-body')),
		...
		heatMapDialog.getWidget().find('.modal-body').append(svg);
		heatMapDialog.show();
