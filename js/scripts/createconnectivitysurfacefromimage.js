...
program.version('1.0.0')
    ...
    .option('--left-range <first:last>','left source-label range',range,[ 501,724 ])
    ...
    .option('--resample <factor>','output spacing relative to input spacing',number,3.0)
...
async function main() {
    ...
    // The legacy vtkpxAnalyzeImageSource uses the stored LPS voxel order.
    await image.load(program.input,false);
