...
const int MAX_STEPS = 443;  // 887 for 512^3, 1774 for 1024^3
...
void main() {
    ...
    float b=gl_FragColor.a*u_opacity;
    gl_FragColor.a= b;
...
float sample_3d_texture(vec3 texcoords) {
  /* Sample float value from a 3D texture. Assumes intensity data. */
  if (texcoords.z>=u_boundsmin.z)
    if (texcoords.y>=u_boundsmin.y)
      if (texcoords.x>=u_boundsmin.x)
          if (texcoords.z<=u_boundsmax.z)
...
vec4 add_lighting(float val, vec3 loc, vec3 step, vec3 view_ray)
{
    ...
    // calculate normal vector from gradient
        vec3 N;
    float val1, val2;
    val1 = sample_3d_texture(loc + vec3(-step[0], 0.0, 0.0));
    val2 = sample_3d_texture(loc + vec3(+step[0], 0.0, 0.0));
    N[0] = val1 - val2;
    val = max(max(val1, val2), val);
    val1 = sample_3d_texture(loc + vec3(0.0, -step[1], 0.0));
    val2 = sample_3d_texture(loc + vec3(0.0, +step[1], 0.0));
    N[1] = val1 - val2;
    val = max(max(val1, val2), val);
    val1 = sample_3d_texture(loc + vec3(0.0, 0.0, -step[2]));
    val2 = sample_3d_texture(loc + vec3(0.0, 0.0, +step[2]));
    N[2] = val1 - val2;
    val = max(max(val1, val2), val);
